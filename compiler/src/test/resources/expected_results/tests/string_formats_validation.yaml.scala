...
import Generators._
    ...
    class String_formats_validationYamlSpec extends Specification {
    ...
    "POST /string" should {
        def testInvalidInput(input: (String, StringPostPassword_optional, LocalDate, StringPostBinary_optional, StringPostDate_optional, Base64String, StringPostBase64optional, StringPostString_optional, DateTime, String, StringPostDate_time_optional)) = {
            ...
            val propertyList = acceptHeaders.map { acceptHeader =>
                ...
                ("given 'Accept' header '" + acceptHeader + "' and URL: [" + url + "]" + "and body [" + parsed_binary_optional + "]") |: all(
        ...
        def testValidInput(input: (String, StringPostPassword_optional, LocalDate, StringPostBinary_optional, StringPostDate_optional, Base64String, StringPostBase64optional, StringPostString_optional, DateTime, String, StringPostDate_time_optional)) = {
            ...
            val propertyList = acceptHeaders.map { acceptHeader =>
                ...
                ("given response code " + expectedCode + " and 'Accept' header '" + acceptHeader + "' and URL: [" + url + "]" + "and body [" + parsed_binary_optional + "]") |: all(
    ...
    "POST /string2" should {
        def testInvalidInput(binary_required: BinaryString) = {
            ...
            val propertyList = acceptHeaders.map { acceptHeader =>
                ...
                ("given 'Accept' header '" + acceptHeader + "' and URL: [" + url + "]" + "and body [" + parsed_binary_required + "]") |: all(
                    requestStatusCode_(path) ?= BAD_REQUEST ,
                    requestContentType_(path) ?= Some(acceptHeader),
                    errors.nonEmpty ?= true,
                    all(validations:_*)
        ...
        def testValidInput(binary_required: BinaryString) = {
            ...
            val propertyList = acceptHeaders.map { acceptHeader =>
                ...
                ("given response code " + expectedCode + " and 'Accept' header '" + acceptHeader + "' and URL: [" + url + "]" + "and body [" + parsed_binary_required + "]") |: all(
                    possibleResponseTypes.contains(expectedCode) ?= true,
                    parsedApiResponse.isSuccess ?= true,
                    requestContentType_(path) ?= Some(acceptHeader),
                    errors.isEmpty ?= true
