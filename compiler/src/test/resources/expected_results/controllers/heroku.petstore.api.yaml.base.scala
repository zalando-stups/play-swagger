...
trait HerokuPetstoreApiYamlBase extends Controller with PlayBodyParsing {
    ...
    case class get200(result: Seq[Pet])(implicit val writers: List[Writeable[Seq[Pet]]]) extends getType[Seq[Pet]] { val statusCode = 200 }
    ...
    def getAction[T] = (f: getActionType[T]) => (limit: Int) => getActionConstructor { request =>
    ...
    sealed trait putType[Result] extends ResultWrapper[Result]
    case class put200(result: Null)(implicit val writers: List[Writeable[Null]]) extends putType[Null] { val statusCode = 200 }
    ...
    sealed trait postType[Result] extends ResultWrapper[Result]
    case class post200(result: Null)(implicit val writers: List[Writeable[Null]]) extends postType[Null] { val statusCode = 200 }
    ...
    sealed trait getbyPetIdType[Result] extends ResultWrapper[Result]
    case class getbyPetId200(result: Null)(implicit val writers: List[Writeable[Null]]) extends getbyPetIdType[Null] { val statusCode = 200 }
