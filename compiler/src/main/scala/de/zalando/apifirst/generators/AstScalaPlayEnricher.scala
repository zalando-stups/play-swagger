...
/**
  ...
  */
class ScalaModelEnricher(app: StrictModel) {
  ...
  private val types = app.typeDefs.toSeq
  ...
  private[apifirst] def apply(implicit table: DenotationTable): DenotationTable = {
    (table /: steps) { (denotation, step) => (denotation /: types) { (dd, refType) =>
  ...
  /**
    ...
    * @return
    */
  private def types(typeDef: (Reference, Type))(table: DenotationTable): Denotation = typeDef match {
  ...
  /**
    ...
    * @return
    */
  private def classes(typeDef: (Reference, Type))(table: DenotationTable): Denotation = typeDef match {
    case (ref, t: TypeDef) if !ref.simple.contains("AllOf") && !ref.simple.contains("OneOf") =>
      val traitName = app.discriminators.get(ref).map(_ => Map("name" -> useType(ref, "", "")))
  ...
  /**
    ...
    * @return
    */
  private def traits(typeDef: (Reference, Type))(table: DenotationTable): Denotation = typeDef match {
  ...
  /**
    ...
    * @return
    */
  private def aliases(typeDef: (Reference, Type))(table: DenotationTable): Denotation = typeDef match {
  ...
  private def typeDefProps(k: Reference, t: Type, fields: Seq[Field])(table: DenotationTable): Map[String, Any] = {
    Map(
